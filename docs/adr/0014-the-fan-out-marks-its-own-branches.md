...
Three consequences follow for the skills, and all are load-bearing. `reviewing-changes` and
...
The third is an ordering constraint that is easy to get backwards, and did start out
backwards. `dispatch` must **commit before it reviews**. A decision pins itself to the sha
that is current when it is recorded, and executors stage rather than commit, so reviewing the
staged diff records the branch point; the orchestrator's own commits then stale it and the
gate refuses every pull request. The fix is ordering, not new machinery: curate the commits
inside the worktree, then review, then push. Anyone reordering those steps back will find the
happy path blocked with a message that looks like the gate malfunctioning.
...
Three costs are accepted rather than solved, in the same shape as ADR 0012's.

An orchestrator that hand-rolls `git worktree add` marks nothing, and its fan-out lands
ungated. That fails open toward the ordinary path, which ADR 0013 already decided needs no
gate, and the skill says plainly why the script is not optional.

Teardown deletes the marker, so a branch merged by hand *after* its worktree is gone is
ungated too. This is the second failure the issue's own table predicted for any
worktree-shaped proxy — "`dispatch` lands from the main checkout after tearing worktrees
down" — and the design does not escape it, it only narrows it. What keeps it small is that
the pull request opens before teardown, so the gate has already fired at the moment that
matters; what is left unguarded is a later merge of an already-reviewed branch. The
protection there is step ordering in a skill, which is tier 1, the tier that fails. Naming
it here is the honest alternative to claiming the proxy problem was solved.

The gate also still refuses to read the decision text, so a fan-out can be recorded as
`skipped` — unchanged from ADR 0011, which conceded that a local check judging the word
would be theatre.
