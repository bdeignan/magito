# Adopt an ADR log; defer docs/agents/GLOSSARY.md

magito ships `domain-modeling` guidance telling projects to record decisions in
`docs/adr/` and domain terms in `docs/agents/GLOSSARY.md`, but kept neither for itself — so real
...
decisions arise rather than backfilling past ones. We defer `docs/agents/GLOSSARY.md` until a domain
