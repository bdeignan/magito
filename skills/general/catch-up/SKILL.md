...
3. **Work in flight** — open issues from the configured tracker (`bash ../implement-issue/scripts/issues.sh list`, or `.scratch/`), and the last handoff for this repo at `~/.magito/handoffs/<repo-slug>.md` (repo root path with every `/` replaced by `-`), if it exists.
